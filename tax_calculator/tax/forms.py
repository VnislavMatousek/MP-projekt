...
class TaxForm(forms.Form):
    ...
    ZTP_CHOICES = [
        ...
        (
            "pecujici_manžel",
            "Sleva na manželku ZTP/P pečující o dítě do 3 let s příjmem nepřesahujícím 68 000 Kč ročně",
        ),
        (
            "ztp_p_pecujici",
            "Sleva na manžela ZTP/P pečující o dítě do 3 let s příjmem nepřesahujícím 68 000 Kč ročně",
        ),
    ]
    ...
    invalidita = forms.ChoiceField(
        choices=INVALIDITA_CHOICES, label="Sleva pro lidi s invalidy"
    )
    ztp = forms.ChoiceField(
        choices=ZTP_CHOICES, label="Sleva pro partnera se ZTP/P průkazem"
    )
...
class TaxFormRok(forms.Form):
    ...
    invalidita1 = forms.ChoiceField(
        choices=INVALIDITA_CHOICES1, label="Sleva pro lidi s invaliditou"
    )
    ztp1 = forms.ChoiceField(
        choices=ZTP_CHOICES1, label="Sleva pro partnera se ZTP/P průkazem"
    )
...
class HistoryFilterForm(forms.Form):
    ...
    sort_by = forms.ChoiceField(
        choices=SORT_CHOICES, required=False, label="Seřadit podle:"
    )
    search_date = forms.DateField(
        required=False,
        widget=forms.DateInput(attrs={"type": "date"}),
        label="Vyhledat datum:",
    )
